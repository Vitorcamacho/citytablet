...
angular.module('myApp2.controllers', [])
    .controller('RoutesCtrl', ['$scope', '$rootScope', '$routeParams', '$window', '$location', 'routeService', function ($scope, $rootScope, $routeParams, $window, $location, routeService) {
        if(map != undefined){
            setMapBearing(new plugin.google.maps.LatLng(32.738337, -16.989902), 17, null);
        }
...
function drawWaytoRoute(map_obj, points, color, opa){
    console.log("drawWaytoRoute");
    map_obj.routes = new Array();
    all_ways = new Array();
    var index = 0;
    if(points['map_routes'].length-1 > 3){
        var length = 3;}
    else
...
function setMapData(init){
    ...
    map.addMarker({
        ...
        if(init)
            ...
            setMapBearing(location,18,null);
...
function initializeRoutesData(scope, idlanguage) {
    ...
    var data_routeinfo = window.sessionStorage.getItem("routes_info");
...
function initialize(service){
    ...
    angular.element(document).ready(function () {
        ...
        var div_menu = document.getElementById("map-aux");

        if (div == undefined || div_menu == undefined) {
...
//            map.addEventListener(plugin.google.maps.event.MAP_LOADED, function () {
//                run_geolocator(service);
//            });
        }
    });
